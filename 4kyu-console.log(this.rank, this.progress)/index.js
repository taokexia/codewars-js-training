...
// activity's rank
// The progress earned from the completed activity is relative to what the user's current rank is compared 
...
// Any remaining progress earned while in the previous rank will be applied towards the next rank's p
// rogress (we don't throw any progress away). The exception is if there is no other rank left to progress 
...
// If a user ranked -8 completes an activity ranked -4 they will receive 160 progress, resulting in the 
// user being upgraded to rank -7 and having earned 60 progress towards their next rank
// If a user ranked -1 completes an activity ranked 1 they will receive 10 progress (remember, zero rank 
//   is ignored)
// Code Usage Examples:
//   var user = new User()
//   user.rank // => -8
...
// Note: Codewars no longer uses this algorithm for its own ranking system. It uses a pure Math based 
// solution that gives consistent results no matter what order a set of ranked activities are completed at.
...
class User {
  ...
  incProgress(level) {
    if(level > 8 || level < -8 || level % 1 !== 0 || level === 0) throw Error()
    console.log(level)
    console.log(this.rank, this.progress)
    var n
    if(level * this.rank < 0)  level > 0 ? n = level - this.rank - 1 : n = level - this.rank + 1
    else n = level - this.rank
    console.log(n)
    var point = 0
    if(n === 0) point = 3
    else if(n === -1) point = 1
    else if(n >0) point = 10 * n * n
    this.progress += point
